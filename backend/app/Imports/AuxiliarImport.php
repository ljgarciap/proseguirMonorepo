...
class AuxiliarImport implements ToModel
{
    ...
    public function model(array $row)
    {
        ...
        if (empty($fechaRaw) || 
            ...
            str_contains($lowercaseFecha, 'auxiliar') ||
            str_contains($lowercaseFecha, 'comprobante') ||
        ...
        $detalle = trim($row[4] ?? '');
        $debito = $this->cleanNumeric($row[8] ?? 0);
        $credito = $this->cleanNumeric($row[9] ?? 0);

        // Generate a unique hash including the row number to allow identical trans within same file
        $uniqueHash = md5($fecha . $co . $documento . $detalle . $debito . $credito . $this->rows);

        $auxiliar = ContableAuxiliar::updateOrCreate(
            ...
            [
                ...
                'documento' => $documento,
                'detalle' => $detalle,
                'centro_costos' => $row[5] ?? null,
                'nit' => $row[6] ?? null,
                'base_local' => $this->cleanNumeric($row[7] ?? 0),
                'debito_local' => $debito,
                'credito_local' => $credito,
                'saldo_local' => $this->cleanNumeric($row[10] ?? 0),
            ]
        );
