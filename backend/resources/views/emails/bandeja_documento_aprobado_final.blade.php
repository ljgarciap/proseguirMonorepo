...
<head>
    <meta charset="utf-8">
    <title>Documento aprobado - Ruta finalizada - Proseguir</title>
    <style>
        ...
        .highlight-box { padding: 16px 20px; margin: 20px 0; border-radius: 8px; font-weight: 700; text-align: center; background-color: #f0fdf4; border-left: 4px solid #16a34a; color: #166534; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>Estimado(a):</p>
            <p>El documento que envió a través de la Bandeja Interna de Documentos completó su ruta de aprobación.</p>
            <div class="highlight-box">APROBADO — RUTA FINALIZADA</div>
            ...
            <p style="margin-bottom:6px;"><strong>Mensaje original</strong></p>
            ...
            <table class="detalle">
                <tr><td class="label">Aprobó</td><td>{{ $step->usuario->name ?? '—' }} ({{ $step->usuario->documentType->nombre ?? 'Documento' }} {{ $step->usuario->numero_documento ?? '—' }})</td></tr>
                <tr><td class="label">Rol de quien aprobó</td><td>{{ $step->area->nombre ?? '—' }}</td></tr>
                <tr><td class="label">Fecha y hora de aprobación</td><td>{{ optional($step->fecha_procesamiento?->bogota())->format('d/m/Y H:i') ?? '—' }}</td></tr>
                <tr><td class="label">Estado</td><td>Aprobado — Ruta finalizada</td></tr>
            </table>
