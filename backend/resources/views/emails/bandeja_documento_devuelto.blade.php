...
<head>
    <meta charset="utf-8">
    <title>Documento devuelto - Proseguir</title>
    <style>
        ...
        .header { background-color: #7f1d1d; background: linear-gradient(135deg, #7f1d1d 0%, #dc2626 100%); padding: 30px; text-align: center; color: #ffffff; }
        .header h1 { margin: 0; font-size: 24px; font-weight: 700; }
        .content { padding: 30px; line-height: 1.6; }
        .content p { margin: 0 0 15px 0; font-size: 16px; color: #4a5568; }
        .highlight-box { padding: 16px 20px; margin: 20px 0; border-radius: 8px; font-weight: 700; text-align: center; background-color: #fef2f2; border-left: 4px solid #dc2626; color: #991b1b; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>Estimado(a):</p>
            <p>El documento que envió a través de la Bandeja Interna de Documentos fue devuelto.</p>
            <div class="highlight-box">DOCUMENTO DEVUELTO</div>
            ...
            <table class="detalle">
                <tr><td class="label">Devuelto por</td><td>{{ $step->area->nombre ?? '—' }}</td></tr>
                <tr><td class="label">Fecha y hora de devolución</td><td>{{ optional($step->fecha_procesamiento?->bogota())->format('d/m/Y H:i') ?? '—' }}</td></tr>
                <tr><td class="label">Motivo de la devolución</td><td>{{ $step->observacion ?? '—' }}</td></tr>
            </table>
