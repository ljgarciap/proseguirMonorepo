...
**Gotcha del cliente de prueba compartido (hallado en sesión SCRUM-339, 2026-09-08):** doc `2345`
es el único usuario portal `cliente` autenticable para e2e, y todos los fixtures de este directorio
lo reutilizan como `cliente_id` de sus `DocumentRequest`. `DocumentRequestController::activeRequest()`
tiene un fallback real de producción ("si ningún `DocumentRequest` pendiente calza con el crédito
que está esperando al cliente ahora mismo, mostrar el más reciente pendiente de ese cliente") — con
varios fixtures dejando `DocumentRequest` en `pendiente` para el mismo cliente 2345, ese fallback le
puede mostrar a un test los documentos de OTRO fixture en `/client-upload` (Mis Cargas), no los
propios. Todo fixture nuevo que cree un `DocumentRequest` para doc 2345 debe cancelar primero
cualquier otro `pendiente` de ese mismo cliente (ver `seed_scrum_193_205.php` o `seed_scrum_339.php`
para el patrón) — si no, un spec puede fallar en Mis Cargas por datos de un fixture ajeno, no por un
bug real.
